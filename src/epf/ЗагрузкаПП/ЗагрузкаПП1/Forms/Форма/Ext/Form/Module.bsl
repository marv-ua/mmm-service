...
Процедура ВыбратьФайлЗавершение(ПомещенныеФайлы, ПустойПараметр) Экспорт
	...
	Если Не имяФайла = ПомещенныеФайлы.СсылкаНаФайл.Имя ИЛИ НачальнаяСтрока = 0 Тогда
		ОпределитьНомераКолонок(ТабДок);
	КонецЕсли;
	имяФайла = ПомещенныеФайлы.СсылкаНаФайл.Имя;
...
&НаСервере
Процедура ОпределитьНомераКолонок(ТабДок)
