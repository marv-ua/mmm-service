...
Функция ПодготовитьПараметрыОтчета()
	...
	О = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].Значение;
	Если О = Неопределено Тогда
		Т = мммСервер.ПолучитьТаблицуСерверов(Отчет.ПериодРегистрации);
		О = Т.ВыгрузитьКолонку("Организация");
	КонецЕсли;
	
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("Организация",
		//Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[1].Значение..ВыгрузитьЗначения()()
		?(ТипЗнч(О) = Тип("СписокЗначений"), О.ВыгрузитьЗначения(), ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(О))
	);
