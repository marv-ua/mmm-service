...
Процедура СформироватьОтчетОСВ(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Для Каждого ВыборкаДетальныеЗаписи Из ТаблицаБаз Цикл
		...
		Попытка
			...
			Запрос.Параметры.Вставить("Счета", РегистрыСведений.Меппинг.ПолучитьЗамену(ВыборкаДетальныеЗаписи.Ссылка, ПараметрыОтчета.Счета[0]));
...
Процедура Обмен8() Экспорт
	...
	ПериодНачала = Константы.ПериодОбновленияДанных.Получить();
	Если Не ЗначениеЗаполнено(ПериодНачала) Тогда
		Возврат;
	КонецЕсли;
	ТекМесяц = ПериодНачала;
	Пока ТекМесяц < ТекущаяДата() Цикл
		ЗаписьЖурналаРегистрации("ОбновлениеДанных.Остатки8", УровеньЖурналаРегистрации.Примечание,,, ТекМесяц);
		ЗаполнитьОСВИз8(ПодготовитьПараметрыОтчета(НачалоМесяца(ТекМесяц)), АдресХранилища);
...
Функция ПодготовитьПараметрыОтчета(Период = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	МАКСИМУМ(АктуальностьОрганизаций.Период) КАК ПериодАктуальности
	               |ПОМЕСТИТЬ ВТОрганизацииПериодыСвязи
	               |ИЗ
	               |	ВТОрганизацииПериоды КАК Организации
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АктуальностьОрганизаций КАК АктуальностьОрганизаций
	               |		ПО Организации.Период >= АктуальностьОрганизаций.Период
	               |			И Организации.Организация = АктуальностьОрганизаций.Организация
	               |ГДЕ
	               |	НЕ Организации.Организация.ЭтоГруппа
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	Организации.Период,
	               ...
	               |	Организации.Период КАК Период,
	               |	Организации.Организация КАК Организация,
	               |	АктуальностьОрганизаций.Активна КАК Активна
	               |ПОМЕСТИТЬ ВТОрганизации
	               |ИЗ
	               |	ВТОрганизацииПериодыСвязи КАК Организации
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АктуальностьОрганизаций КАК АктуальностьОрганизаций
	               |		ПО Организации.ПериодАктуальности = АктуальностьОрганизаций.Период
	               |			И Организации.Организация = АктуальностьОрганизаций.Организация
	               |			И (АктуальностьОрганизаций.Активна)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("НачалоПериода", Константы.ПериодОбновленияДанных.Получить());
	Запрос.УстановитьПараметр("КонецПериода", КонецМесяца(Константы.ПериодОбновленияДанных.Получить()));
	тз1 = Запрос.Выполнить().Выгрузить();
	
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("Организация"                       , тз1.ВыгрузитьКолонку("Организация"));
	ПараметрыОтчета.Вставить("НачалоПериода"                     , Константы.ПериодОбновленияДанных.Получить());
...
Процедура ЗаполнитьОСВИз8(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ПараметрыОтчета.ОбновитьДанные Тогда
		...
		Для Каждого База Из ТаблицаБаз Цикл
			...
			Если Не ЗначениеЗаполнено(База.Сервер) Или Не ЗначениеЗаполнено(База.База) Тогда
				Продолжить;
			КонецЕсли;
