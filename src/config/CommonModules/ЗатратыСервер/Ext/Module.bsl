...
Функция ВыпискаПроверитьСтроку(Данные, ДокВыписка, ДопПараметр, ДелатьЗеркальнуюОперацию = Ложь) Экспорт
	...
	ИначеЕсли ИСТИНА
		...
		И СтрНайти(ВРег(ДокВыписка.Субконто1), Врег("премия")) > 0 Тогда
		Возврат Истина;
...
|	КонтрагентыДополнительныеРеквизиты.Значение.Наименование КАК ТипКлиента,")+"
	...
	|					КОНЕЦ
	|			КОНЕЦ
	|	КОНЕЦ КАК СТРОКА(100)) = ""Офис"" ТОГДА ИСТИНА ИНАЧЕ ")+"
|	ВЫБОР
|		КОГДА ТИПЗНАЧЕНИЯ(ХозрасчетныйОбороты.КорСубконто1) = ТИП(Справочник.Контрагенты)
|			ТОГДА ВЫБОР
...
|						ТОГДА ИСТИНА
|				КОНЕЦ
|	КОНЕЦ "+?(БазаУТП, "", "КОНЕЦ ")+"КАК АРЕНДА,
	...
	|					КОНЕЦ
	|			КОНЕЦ
	|	КОНЕЦ КАК СТРОКА(100)) = ""Поставщики товара"" ТОГДА ИСТИНА ИНАЧЕ ")+"
|	ВЫБОР
|		КОГДА ТИПЗНАЧЕНИЯ(ХозрасчетныйОбороты.КорСубконто1) = ТИП(Справочник.Контрагенты)
|			ТОГДА ВЫБОР
...
|						ТОГДА ИСТИНА
|				КОНЕЦ
|	КОНЕЦ "+?(БазаУТП, "", "КОНЕЦ ")+" КАК ПОСТАВЩИКИ,
|	ВЫБОР
|		КОГДА ТИПЗНАЧЕНИЯ(ХозрасчетныйОбороты.КорСубконто1) = ТИП(Справочник.Контрагенты)
|			ТОГДА ВЫБОР
...
|
	...
	|		ПО (УпрАналитика1.Объект = ХозрасчетныйОбороты.КорСубконто1)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДопСвойство КАК УпрАналитика2
	|		ПО (УпрАналитика2.Объект = ХозрасчетныйОбороты.КорСубконто1.Родитель)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДопСвойство КАК УпрАналитика3
	|		ПО (УпрАналитика3.Объект = ХозрасчетныйОбороты.КорСубконто1.Родитель.Родитель)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДопСвойство КАК УпрАналитика4
	|		ПО (УпрАналитика4.Объект = ХозрасчетныйОбороты.КорСубконто1.Родитель.Родитель.Родитель)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДопСвойство КАК УпрАналитика5
	|		ПО (УпрАналитика5.Объект = ХозрасчетныйОбороты.КорСубконто1.Родитель.Родитель.Родитель.Родитель)
|
|")+"
|СГРУППИРОВАТЬ ПО 
|"+?(БазаУТП, "", "
...
|				КОНЕЦ
|	КОНЕЦ,
|"+?(БазаУТП, "","КонтрагентыДополнительныеРеквизиты.Значение.Наименование,")+"
...
Процедура Запрос51(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет, Курс)
	...
	Запрос.Параметры.Вставить("ДатаОкончания", Новый Граница(КонецДня(ПараметрыОтчета.КонецПериода), ВидГраницы.Включая));
	Запрос.Параметры.Вставить("Организация",   Организация.ИНН);
	Запрос.Параметры.Вставить("КорСчет", РегистрыСведений.Меппинг.ПолучитьЗамену(Организация, "51"));//ПланыСчетов.Хозрасчетный.ПереводыВПути_);
	Запрос.Параметры.Вставить("Счет", РегистрыСведений.Меппинг.ПолучитьЗамену(Организация, "51"));//ПланыСчетов.Хозрасчетный.РасчетныеСчета);
	Запрос.ТекстЗапроса = "
		|ВЫБРАТЬ
		|	Хозрасчетный.Ссылка КАК Счет
