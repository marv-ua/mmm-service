...
Процедура СформироватьЭтотОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Для Каждого Стр Из ПараметрыОтчета.Итоги.ОстаткиОбороты Цикл
		...
		Если ПараметрыОтчета.РасширенныеИтоги Тогда
			...
			ОбластьДоп.Параметры.ЗатратыМлы = ?(Не ЗначениеЗаполнено(ОбластьДоп.Параметры.Затраты), 0, ОбластьДоп.Параметры.Затраты) + Стр.РасходМатериалы;
...
Процедура НеВошедшиеКонтрагентыПоУслугам(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет, Курс)
	...
	ВыполняемыйКод = "ГруппаКонтрагентовЗатраты=Справочники.Контрагенты.НайтиПоНаименованию(""ЗАТРАТЫ"",Истина,,);
		...
		|ВыбСчет.Добавить(ПланыСчетов.Хозрасчетный."+РегистрыСведений.Меппинг.ПолучитьЗамену(Организация, "РасчетыСПоставщикамиИПодрядчиками")+");
