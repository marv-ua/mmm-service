...
Процедура СформироватьЭтотОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	//Если ТабВНД.Количество() Тогда
		...
		Для Каждого Стр Из ТабВНД Цикл
			Если Стр.Сумма.Приход = 0 И Стр.СуммаРасход = 0 Тогда
				Продолжить;
			КонецЕсли;
			Область.Параметры.Заполнить(Стр);
			Т.Вывести(Область);
	...
	ТабличныйДокумент.ВставитьОбласть(Т.Область(1, 1, Т.ВысотаТаблицы, Т.ШиринаТаблицы), ТабличныйДокумент.Область(1, 35), ТипСмещенияТабличногоДокумента.ПоГоризонтали);
	ТабличныйДокумент.ВставитьОбласть(ТТ.Область(1, 1, ТТ.ВысотаТаблицы, ТТ.ШиринаТаблицы), ТабличныйДокумент.Область(1, 52), ТипСмещенияТабличногоДокумента.ПоГоризонтали);
...
Функция СформироватьТабличноеПредставление(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет, Группа, ДанныеВыборки, НомерПП) Экспорт
	...
	ОбластьМакетаВсего.Параметры.ИтСуммаВсего = ИтогоСуммаЗПИЛНалоги + ТаблицаУслуг.Итог("СуммаУслуг") + ТаблицаМатериалов.Итог("СуммаПоступленийМатериалов");
	...
	ИтСуммаВсегоПоВсем = ИтогоСуммаЗПИЛНалоги +
		...
		ТаблицаКонтрагентыДопРеквизиты.Итог("СуммаУслуг") + 
...
|	NULL
|ИЗ
|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Месяц, Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Касса)), , "+?(БазаУТП, "", "Организация.ИНН = &Организация")+", НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Касса)), ) КАК ХозрасчетныйОбороты
|"+?(БазаУТП, "", "
...
Процедура НеВошедшиеКонтрагентыПоУслугам(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет, Курс)
