...
Процедура СформироватьЭтотОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Для Каждого ВыборкаДетальныеЗаписи Из ТаблицаОрганизаций Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(Окр(100*Прогон/ТаблицаОрганизаций.Количество()), СокрЛП(ВыборкаДетальныеЗаписи.Ссылка.НаименованиеСокращенное),);
...
Функция ВыпискаПроверитьСтроку(Данные, ДокВыписка, ДопПараметр, ДелатьЗеркальнуюОперацию = Ложь) Экспорт
	...
	ИначеЕсли ИСТИНА //СтрНайти(Содержание, "Оплата от покупателей") > 0 
		...
		И Сумма > 0
		И (ДокВыписка.ВидДвижения = "Поступления от покупателей" ИЛИ ДокВыписка.ВидДвижения = "Прочие поступления" ИЛИ ДокВыписка.ВидДвижения = "ВНД")
