...
Процедура ЗагрузитОстаткиИОбороты7(Период, ПоказыватьПредупреждения = Истина) Экспорт
	...
	#Если Сервер Тогда
		//Параметры = ВнешниеИсточникиДанных.МММ.ПолучитьОбщиеПараметрыСоединения();
		//Параметры.АутентификацияСтандартная = Истина;
		//Параметры.ИмяПользователя = "Robot";
		//Параметры.Пароль = "bot";
		//Параметры.СтрокаСоединения = "Driver={Microsoft dBase Driver (*.dbf)};Dbq="+Константы.ПутьКБазеМММ.Получить()+";";
		//Параметры.СУБД = «MSSQLServeг»;
		Connection = Новый COMОбъект("ADODB.Connection");
		Path=Константы.ПутьКБазеМММ.Получить();
	...
	#Иначе
		...
		ОтчетОСВВызовСервера.ОчиститьРегистрОбороты7(НачалоМесяца(Период));
...
Процедура ПолучитьОстаткиИобороты51Из7(БазаОле, Период)
	...
	ОтчетОСВВызовСервера.ЗаписатьВРегистрОбороты7(тч);
