...
Процедура ПериодРегистрацииПриИзменении()
	...
	Очередь.Параметры.УстановитьЗначениеПараметра("ПериодЗатратКонец", КонецМесяца(Объект.ПериодРегистрации));
	
	Если ОбновлениеЗаПериод Тогда
		Очередь.Параметры.УстановитьЗначениеПараметра("ПериодЗатрат", ПериодОбновления.ДатаНачала);
		Очередь.Параметры.УстановитьЗначениеПараметра("ПериодЗатратКонец", ПериодОбновления.ДатаОкончания);
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
&НаСервере
Функция ЗапуститьВыполнениеВФоне(ИдентификаторФормы, Многопоточно = Ложь, ОбновлениеЗаПериод = Ложь)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(Элементы.Результат, "НеИспользовать");
	ПараметрыОтчета = ПодготовитьПараметрыОтчета(ОбновлениеЗаПериод);
...
Процедура ФоновоеЗаполненияЗавершено(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		ПолноеИмяФайла = Каталог+Формат(Объект.ПериодРегистрации, "ДФ=гггг_ММ_")+СокрЛП("НМУ_"+ИмяОрганизации)+"_"+Формат(ТекущаяДата(), "ДФ=yy-MM-dd")+".xlsx";

		Попытка
			ТаблДокумент.Записать(ПолноеИмяФайла, ТипФайлаТабличногоДокумента.XLSX);

			Сообщить("Файл отчета успешно сформирован. Путь к файлу: "+ПолноеИмяФайла);
		Исключение
		КонецПопытки;
		
		Рез.Лог.Показать(СтрШаблон("Затраты: %1", ТекущаяДата()));
...
&НаСервере
Функция ПодготовитьПараметрыОтчета(ОбновлениеЗаПериод = Ложь)
	...
	ПараметрыОтчета.Вставить("РасширенныеИтоги", РасширенныеИтоги);
	ПараметрыОтчета.Вставить("Организация", Организация.ВыгрузитьЗначения());
	ПараметрыОтчета.Вставить("НачалоПериода", ?(ОбновлениеЗаПериод, ПериодОбновления.ДатаНачала, НачалоДня(Объект.ПериодРегистрации)));
	ПараметрыОтчета.Вставить("КонецПериода", ?(ОбновлениеЗаПериод, ПериодОбновления.ДатаОкончания, КонецМесяца(Объект.ПериодРегистрации)));
	ПараметрыОтчета.Вставить("ОбновлениеЗаПериод", ОбновлениеЗаПериод);
	...
	Если ОбновлениеЗаПериод Тогда
		ПараметрыОтчета.НеФормироватьДанныеДляВыгрузки = Ложь;
		ПараметрыОтчета.ТолькоДанныеВыгрузки = Ложь;
		ПараметрыОтчета.РасширенныеИтоги = Ложь;
	КонецЕсли;
...
Процедура ВопросОтправкиВ7(Ответ, ДопПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Парам = Новый Структура("ВыделенныеСтроки,ОчищатьДокументыПередВыгрузкой,НеВыгружатьКассу", ДопПараметры, ОчищатьДокументыПередВыгрузкой, НеВыгружатьКассу);
		
		Если ЗатратыСерверПереопределяемый.ПодключатьсяК7НаСервере() Тогда
			ОтправитьДанныеВ7НаСервере(Парам);
		Иначе
			ЗатратыКлиент.ОтправитьДанныев7(Парам, ЭтаФорма, Элементы);
		КонецЕсли;
		ПериодРегистрацииПриИзменении();
		ОбновитьОтображениеДанных();
	КонецЕсли;
...
&НаСервере
Процедура ОтправитьДанныеВ7НаСервере(ДопПараметры)
...
&НаКлиенте
Процедура ОбновитьДанныеЗаПериод(Команда)

	ОбновлениеЗаПериод = Истина;
	Обработчик = Новый ОписаниеОповещения("ВыбратьПериодЗавершение", ЭтотОбъект);
	Диалог = Новый ДиалогРедактированияСтандартногоПериода;
	Диалог.Период = ПериодОбновления;
...
Процедура ВыбратьПериодЗавершение(Результат, ДопПараметры) Экспорт
	...
	ПериодОбновления = Результат;
	
	ОчиститьСообщения();

	СтруктураФоновогоЗадания = ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,, Истина);
