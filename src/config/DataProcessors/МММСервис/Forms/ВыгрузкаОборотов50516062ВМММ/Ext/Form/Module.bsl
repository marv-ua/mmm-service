...
Функция ЗапуститьВыполнениеВФоне(ИдентификаторФормы, Многопоточно = Ложь)
	...
	Возврат ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, 
		"ОтчетОСВВызовСервера.СформироватьЭтотОтчет", 
		ПараметрыОтчета, 
		АдресХранилища
	);	
...
Процедура ФоновоеЗаполненияЗавершено(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		Для Каждого Д Из Данные Цикл
			...
			мКаталогСохранения = КаталогСохраненния + ПолныйПутьСохранения;
			Если ПроверитьСуществованиеКаталога(мКаталогСохранения) = ложь Тогда
				СоздатьКаталог(мКаталогСохранения);
			КонецЕсли;
			
			ПолноеИмяФайлаXLS =мКаталогСохранения+СокрЛП(Формат(Объект.ПериодРегистрации,"ДФ=yyyy_MM_")+"АС_50_62_"+ИмяОрганизации)+"_"+Формат(ТекущаяДата(), "ДФ=dd-MM-yy")+".xls";
			ПолноеИмяФайлаPDF =мКаталогСохранения+СокрЛП(Формат(Объект.ПериодРегистрации,"ДФ=yyyy_MM_")+"АС_50_62_"+ИмяОрганизации)+"_"+Формат(ТекущаяДата(), "ДФ=dd-MM-yy")+".PDF";
	...
	Рез.Лог.Показать(СтрШаблон("Обороты: %1", ТекущаяДата()));
...
&НаКлиенте
Процедура КаталогСохраненнияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Диалог.Выбрать() Тогда
		КаталогСохраненния = Диалог.Каталог;
		ОбновитьПутьСохранения();
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьПутьСохранения()

	ПолныйПутьСохранения = "";
	ПолныйПутьСохранения = "\"+Формат(Объект.ПериодРегистрации, "ДФ=yyyy_MM")+
		?(Организация.Количество() = 1, "\"+Организация[0].Значение, "")+"\";
